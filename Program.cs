...
class Program
{
    static void Main(string[] args)
        ...
        string json = JsonSerializer.Serialize(classJobsData, options);
            ...
            string filePath = Path.Combine(Directory.GetCurrentDirectory(), "patches/6.58/ClassJobs.json");

            File.WriteAllText(filePath, json);

            Console.WriteLine("ClassJobs data extracted and saved to ClassJobs.json");
        }
}
